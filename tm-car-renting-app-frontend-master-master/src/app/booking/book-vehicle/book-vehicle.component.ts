  ...
  filterVehicleList(event): void {
    ...
    if (selectedvalue !== '') {
      this.availableVehiclesForBookingFiltered =
        this.availableVehiclesForBooking.filter(f => f.Vehicle_SubCategoryId === Number(selectedvalue)).slice();
    } else {
  ...
  bookVehicle(vehicelID: number): void {
    this.authService.hasUserloggedIn.subscribe(data => {
      if (!data) {
        ...
        this.bookedVehicle = this.availableVehiclesForBooking.find(f => f.Vehicle_id === vehicelID);
        this.stepper.next()
      }
    });
  }
